...
- **Offline-first**: `Resilience.retry` falla rápido cuando `NetworkMonitor.isOnline == false` (todos los `runCall` de los repos pasan por ahí) — sin este guard cada llamada quemaba ~25s de reintentos y congelaba la app offline. `PanaApplication.observeConnectivityRestore()` reconecta Realtime y dispara `MessagesRepository.scheduleSync()` al volver la señal; `SupabaseClient.scheduleReconnect` no reintenta mientras offline. Chats/mensajes/perfiles ya eran cache-first (Room + `PublicProfileEntity` + Coil disk cache en `filesDir/image_cache`); mensajes pendientes de texto se encolan en Room y los sube `SyncMessagesWorker` con constraint de red.

- **Toolchain de build en sandbox** (2026-08-21): JDK 17 portable en /workspace/toolchain/jdk-17.0.20.1+1 y Android SDK en /workspace/toolchain/sdk (platform 35, build-tools 35/36). Compilar: JAVA_HOME=/workspace/toolchain/jdk-17.0.20.1+1 ANDROID_HOME=/workspace/toolchain/sdk ./gradlew :app:compileDebugKotlin. El build release esta bloqueado por diseno sin KEYSTORE_FILE/KEYSTORE_PASSWORD/KEY_ALIAS (build.gradle.kts ~linea 68). Falta app/google-services.json (gitignored): para compilar basta un JSON dummy con package_name com.panalink.app (borrarlo despues). Los resets de terminal matan procesos: lanzar gradle con setsid nohup ... > /tmp/build.log 2>&1 < /dev/null & y hacer polling del log.
