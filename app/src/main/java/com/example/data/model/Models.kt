...
@JsonClass(generateAdapter = true)
data class ContactWithProfileEntity(
    @Json(name = "id") val id: String = "",
    @Json(name = "owner_user_id") val ownerUserId: String,
    @Json(name = "contact_user_id") val contactUserId: String,
    @Json(name = "created_at") val createdAt: String? = null,
...
data class MessageReaction(
    @Json(name = "thread_message_id") val threadMessageId: String,
    @Json(name = "user_id") val userId: String,
...
data class UserState(
    ...
    @Json(name = "type") val type: String? = "story",
    @Json(name = "created_at") val createdAt: String? = null,
    ...
    @Json(name = "favorited_by_me") val favoritedByMe: Boolean? = false,
    @Json(name = "viewed_by_me") val viewedByMe: Boolean? = false,
    @Json(name = "views_count") val viewsCount: Int? = 0,
    @Json(name = "preview_metadata") val previewMetadata: String? = null,
    val localVideoPath: String? = null
) {
    val userId: String get() = authorId ?: userIdField ?: ""
    val isReel: Boolean get() {
        if (type != null) return type == "reel"
        val cap = caption ?: ""
        return cap.contains("[Transition:") || cap.contains("[CoverFrame:")
    }
}
...
data class SignUpRequest(
    ...
    @Json(name = "options") val options: SignUpOptions? = SignUpOptions()
)
...
@JsonClass(generateAdapter = true)
data class AddContactByPinResponse(
    @Json(name = "success") val success: Boolean,
    @Json(name = "contact_id") val contactId: String,
    @Json(name = "display_name") val displayName: String,
    @Json(name = "avatar_url") val avatarUrl: String?,
    @Json(name = "thread_id") val threadId: String,
    @Json(name = "is_already_contact") val isAlreadyContact: Boolean? = false
)

@JsonClass(generateAdapter = true)
data class ContactIdentifierResponse(
...
fun formatIsoDateTime(isoStr: String?): String {
    ...
    return try {
        val cleanStr = if (isoStr.contains(".")) {
            isoStr.substringBefore(".")
        } else if (isoStr.contains("+")) {
            isoStr.substringBefore("+")
        } else if (isoStr.endsWith("Z")) {
            isoStr.substringBefore("Z")
        } else {
            isoStr
        }
...
data class SearchStickersRequest(
    ...
    @Json(name = "limit") val limit: Int = 24
)
...
data class ChannelComment(
    @Json(name = "id") val id: String = "",
    @Json(name = "chat_message_id") val chatMessageId: String,
    @Json(name = "chat_id") val chatId: String,
    ...
    @Json(name = "edited_at") val editedAt: String? = null,
    @Json(name = "deleted_at") val deletedAt: String? = null,
    @Json(name = "author") val author: Profile? = null
)
...
data class PresenceSession(
    ...
    @Json(name = "location_approx") val locationApprox: String? = null,
    @Json(name = "isp_name") val ispName: String? = null,
